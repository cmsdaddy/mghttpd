...
def show_alarm_control_page(request):
    ...
    return render(request, "status/01-全局状态及控制.html", context=context)


url_patterns = [
    # 蜂鸣器状态展示
    path('', lambda request: show_alarm_control_page(request)),
    path('status/', lambda request: show_alarm_control_page(request)),
]


urls = (url_patterns, "control", "control")
